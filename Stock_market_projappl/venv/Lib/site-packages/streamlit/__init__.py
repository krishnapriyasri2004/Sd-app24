...
# Session Stateimport streamlit as st
...
def register_user(full_name, email, password, phone_number, address, dob, gender):
    ...
    insert_query = """
        INSERT INTO users (full_name, email, password, phone_number, address, dob, gender)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
    """
    cursor.execute(insert_query, (full_name, email, hashed_password.decode('utf-8'), phone_number, address, dob, gender))
    conn.commit()
    ...
    if st.button("Sign Up"):
        if not full_name:
            ...
            success, message = register_user(
                full_name, email, password, phone_number, address, dob, gender
            )
            if success:
                st.success(message)
                st.session_state['page'] = 'login'  # Go back to login after signup
